% jmsmac.tex 
% $Id: jmsmac.tex,v 1.1 2005/08/23 15:09:00 mike Exp $

% Macros for Mike's book format

%\input jmsdef
% jmsdef.tex 
% $Id: jmsdef.tex,v 1.9 2005/09/05 17:19:36 mike Exp $
...
\def\placepic#1{\medskip\hbox to\hsize{#1}\medskip}
...
\def\listing{\beforedisplay\begingroup
  \blankstrue\setupverbatim\progfont\numlines\dolisting}
...
\def\displaylines{\display
  \halign to\hsize\bgroup
    \hskip\leftskip\hskip\blockindent$\@lign##$\hfil\tabskip=0pt plus1fil\cr}
...
\def\hgreyi{%
  ...
  \setbox1=\hbox{\hbox to\wd0{\strut
    \special{ps:gsave \grey\space setgray}%
    \leaders\vrule\hfill
    \special{ps:grestore}}}%
...
\def\vgreyi{%
  \setbox1=\vbox to0pt{\hbox to\hsize{\negthinspace
    \special{ps:gsave \grey\space setgray}%
    \leaders\hrule height\ht0 depth\dp0\hfill
    \special{ps:grestore}%
    \negthinspace}
   \vss}
  \vbox{\box1\nointerlineskip\box0}}
...
\def\NL{\egroup\penalty100
  \hbox to\hsize{\unhbox0\unskip\hfil}}
\def\RC{\quad\egroup
...
\def\quotation{\startlist\parindent=0pt\listitem{}}
